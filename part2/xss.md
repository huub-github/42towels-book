...
| Embed an XSS payload ``</script><script>alert(`xss`)</script>`` into one of our marketing collaterals.                          | ★★★★★★ |
...
told that the 42 Towels Shop uses a modern _Single Page Application_
...
* There are only some input fields in the 42 Towels Shop forms that validate
  their input.
...
As presented in the
[Architecture Overview](/introduction/architecture.md), the 42 Towels
...
* The mentioned "marketing collateral" might have been publicly
  advertised by the 42 Towels Shop but is not necessarily part of its
  sitemap yet
